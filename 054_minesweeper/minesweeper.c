...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  board_t * gameboard = malloc(sizeof(*gameboard));
  gameboard->width = w;
  gameboard->height = h;
  gameboard->totalMines = numMines;
  gameboard->board = malloc(h * sizeof(*(gameboard->board)));

  for (int i = 0; i < h; i++) {
    gameboard->board[i] = malloc(w * sizeof(*(gameboard->board[i])));
    for (int j = 0; j < w; j++) {
      gameboard->board[i][j] = UNKNOWN;
    }
  }

  for (int i = 0; i < numMines; i++) {
    addRandomMine(gameboard);
  }
...
int countMines(board_t * b, int x, int y) {
  ...
  int x_end = x + 2 <= b->width ? x + 2 : b->width;
  int y_end = y + 2 <= b->height ? y + 2 : b->height;

  for (int i = x_start; i < x_end; i++) {
    for (int j = y_start; j < y_end; j++) {
      counter = IS_MINE(b->board[j][i]) ? counter + 1 : counter;
    }
  }
